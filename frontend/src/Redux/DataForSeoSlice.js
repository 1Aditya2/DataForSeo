...
export const idApiCall = createAsyncThunk(
  ...
  async (body, thunkAPI) => {
    try {
      ...
      if (newresponse == "Task not found") {
      ...
      } else if (newresponse == "Task in queue") {
        thunkAPI.dispatch(
          showToast({
            type: TOAST_FAILURE,
        ...
        const data = {
          ...
          image:
            newresponse?.data.tasks[0]?.result[0]?.items[0]?.meta
              ?.social_media_tags?.["og:image"],
          loadingScore:
          ...
          imagesCount:
            newresponse?.data.tasks[0].result[0].items[0].meta.images_count,
          scripts:
            newresponse?.data.tasks[0].result[0].items[0].meta.scripts_count,
          scriptsSize:
            newresponse?.data.tasks[0].result[0].items[0].meta.scripts_size,
        };
        thunkAPI.dispatch(setallData(data));
        thunkAPI.dispatch(
          setContent(
            newresponse?.data.tasks[0]?.result[0]?.items[0]?.meta.content
          )
        );
        thunkAPI.dispatch(
          setChecks(newresponse?.data.tasks[0]?.result[0]?.items[0]?.checks)
        );
        thunkAPI.dispatch(
          setHtags(newresponse?.data.tasks[0]?.result[0]?.items[0]?.meta.htags)
        );
        thunkAPI.dispatch(
          setSpeed(newresponse?.data.tasks[0]?.result[0]?.items[0]?.page_timing)
        );
        thunkAPI.dispatch(
          setMediaTags(
            newresponse?.data.tasks[0]?.result[0]?.items[0]?.meta
...
const createTask = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(idApiCall.fulfilled, (state, action) => {});
  },
});
